...
class Audio extends Component {
  constructor(props) {
    ...
    this.trackerIsInsideOfSmallCircle = this.trackerIsInsideOfSmallCircle.bind(
      this
    );
    this.trackerIsOusideOfBigCircle = this.trackerIsOusideOfBigCircle.bind(
      this
    );
    // @Controls
  ...
  handleWorkerCallback(workerResponse) {
    ...
    let { audioContextCreatedTime, audioLoadOffsetTime } = this.state;
  ...
  audioStream(url) {
    ...
    fetch(url)
      .then((response) => {
        ...
        const stream = this.readAudioStream(response, contentLength, {
          all: false,
          sec: 3,
          amount: 1245184,
        });
  ...
  preLoadCompleteSong() {
    ...
    new Promise((resolve) => {
      ...
      const stream = this.readAudioStream(
        audioStreamData.response,
        audioStreamData.contentLength,
        { all: true, sec: 1, amount: 1050478 }
      );
  ...
  switchSong(musicIndex) {
    let { tracks, currentSource } = this.state;
    ...
    this.setState(
      { musicIndex, playingFullMusic: false, canLoadFullSong: true },
      () => this.loadSong(tracks[musicIndex].url)
    );
  }
  ...
  framerInit() {
    let { canvasScaleCoef, framerTickSize, framerCountTicks } = this.state;
  ...
  sceneClear() {
    const { canvasWidth, canvasHeight, canvasContext } = this.state;
  ...
  controlsDraw() {
    ...
    if (trackerEnabled || !hasStreamSupport) {
      ...
      canvasContext.arc(
        sceneRadius + scenePadding + x,
        sceneRadius + scenePadding + y,
        10,
        0,
        Math.PI * 2,
        false
  ...
  framerDrawTick(x1, y1, x2, y2) {
    const { canvasCx, canvasCy, canvasContext } = this.state;
    ...
    for (let i = 0, len = ticks.length; i < len; ++i) {
      ...
      if (this.state.gainNode) {
        switch (this.state.gainNode.gain.value) {
          ...
          case 0.5:
            delta =
              (((framerFrequencyData[i] || 0) - lesser * coef) *
                canvasScaleCoef) /
              2;
            break;
          ...
          case 1:
            delta =
              ((framerFrequencyData[i] || 0) - lesser * coef) * canvasScaleCoef;
            break;
  ...
  framerGetTickPoints() {
    const { framerCountTicks, framerPI } = this.state;
    const coords = [],
      step = framerPI / framerCountTicks;
  ...
  framerDrawEdging() {
    ...
    canvasContext.moveTo(
      scenePadding + 2 * sceneRadius - trackerInnerDelta - offset,
      scenePadding + sceneRadius
    );
    canvasContext.arc(
      canvasCx,
      canvasCy,
      sceneRadius - trackerInnerDelta - offset,
      0,
      framerLoadingAngle,
      false
    );
  ...
  trackerDraw() {
    ...
    if (currentSource !== null && this.state.trackerEnabled) {
      ...
      if (!trackerPressButton) {
        const angle =
          ((audioContext.currentTime - audioLoadOffsetTime) /
  ...
  trackerCalculateAngle(event) {
    ...
    let angle = Math.atan(
      (my - canvasCy - canvasCoord.top) / (mx - canvasCx - canvasCoord.left)
    );
  ...
  trackerIsOusideOfBigCircle(event) {
    let { canvasCx, canvasCy, canvasCoord, sceneRadius } = this.state;
    return (
      Math.abs(event.pageX - canvasCx - canvasCoord.left) > sceneRadius ||
    ...
    if (audioContext && audioContext.state !== "suspended" && currentSource) {
      ...
      if (
        ...
        if (threadInUse === "main") {
          ...
          this.audioWorker.postMessage({
            type: "preload",
            data: { playingFullMusic, all: true },
          });
        }
      } else {
        ...
        if (
          playingFullMusic &&
          audioCurrentTime >= currentDuration - 1.5 &&
          !isLoadingFullSong
        ) {
          this.nextSong();
        }
